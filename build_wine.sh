...
if [ -n "${CUSTOM_SRC_PATH}" ]; then
        ...
        if [ "${WINE_VERSION}" = "git" ]; then
                ...
                #git clone https://gitlab.winehq.org/wine/wine.git wine
                git clone -b wine-9.2-custom-test https://github.com/on-that-tiger-mountain/wine-9.2-custom.git wine
                #cd wine
                #git checkout 681d2dfd9d0f6df0a0f34a751663b43369de85b2
                #cd ..
